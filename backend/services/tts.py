...
def _web_search(query: str) -> str:
    ...
    if not api_key:
        # Prefixed so llm.py's system prompt can detect this and refuse to
        # guess, instead of the model quietly narrating around a missing key.
        return "TOOL_UNAVAILABLE: Web search is not configured on this server (missing TAVILY_API_KEY)."
    try:
        res = requests.post(
        ...
        return f"TOOL_UNAVAILABLE: Search error: {e}"
...
def _get_weather(location: str) -> str:
    try:
        ...
        return f"TOOL_UNAVAILABLE: Could not fetch weather for '{location}': {e}"
...
def _wikipedia(topic: str) -> str:
    try:
        ...
        return f"TOOL_UNAVAILABLE: Wikipedia lookup failed for '{topic}': {e}"
...
def run_tool(name: str, args: dict) -> str:
    try:
        ...
        return f"TOOL_UNAVAILABLE: Tool '{name}' failed: {e}"