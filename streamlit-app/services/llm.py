...
def chat_with_memory(user_message: str, history: list, persona: Optional[str] = None) -> tuple[str, list]:
    ...
    bug in our request â€” so we retry once with tools enabled, and if that
    ...
    def _first_call(use_tools: bool):
        kwargs = dict(model="openai/gpt-oss-120b", messages=messages, max_tokens=500, temperature=0.7)
        if use_tools:
            kwargs["tools"] = TOOLS
    ...
    if response is None:
        # Both attempts failed â€” fall back to a plain answer with no tools
